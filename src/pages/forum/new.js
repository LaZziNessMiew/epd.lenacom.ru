...
export default function PalataNew(props) {
    ...
    const dobavitPost = async (e) => {
        e.preventDefault()
        let forumTag = router.query.forum
        let req = await fetch(`/api/forum/posts`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ token: props.token, title, content, forumTag })
        })
        let msg
        if (req.status != 502) {
            ...
            msg = res.msg
            if (req.status == 200) {
                router.push('/forum/' + forumTag + '/' + res.data)
            }
        } else {
                ...
                <div className="palataSubWrapper">
                    ...
                    <Form className="w-100">
                        ...
                        <div className="mobPaddingLR10">
                            <button className="bigBlueButton fullWidthBBB" onClick={(e) => { dobavitPost(e) }}>Добавить тему</button>
                        </div>
                    </Form>
