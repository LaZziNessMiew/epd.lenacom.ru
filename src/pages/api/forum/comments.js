const { Users, ForumPosts, ForumComments } = require('../../../../models/pg')
            ...
            try {
                ...
                if (token, content) {
                    ...
                    if (verify) {
                        ...
                        if (replyComId) {
                            let comment = await ForumComments.findOne({
                        ...
                        await ForumComments.create(data)
                ...
                console.log('ERROR in api/forum/comments:', err)
                res.status(500).json({
            ...
            try {
                ...
                if (postId) {
                    let comments = await ForumComments.findAll({
                        attributes: ['id', 'content', 'create_date', 'upvote', 'downvote', 'reply_com_id', 'reply_user_id', 'nest_lvl'],
                        include: [
                            {
                                model: Users, as: "user", attributes: ['login'],
                            },
                            {
                                model: ForumPosts, as: "forum_post", attributes: ['id'], where: { id: postId }
                            },
                        ],
                        order: [
                            ['nest_lvl', 'ASC'],
                ...
                if (token && commentId && content) {
                    ...
                    if (verify) {
                        ...
                        if (new Date() - comment.create_date < 3000000) {
                            ...
                            await ForumComments.update(data, {
                                where: {
                                    id: commentId
                                }
                            })
        ...
        case 'DELETE':
            try {
                ...
                if (token && commentId) {
                    let verify = verifyToken(token)
                    if (verify) {
                        let post = await ForumComments.findOne({
                            where: {
                                id: commentId
                            },
                        })
                        if (post) {
                            await ForumComments.destroy({
                                where: {
                                    id: commentId
                                }
                            })
